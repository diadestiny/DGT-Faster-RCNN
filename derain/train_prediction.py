...
from nets import MyNet, MyNet_DGNL, MyNet_CFT, DDGN_Depth, DDGN_Depth_CFT, Basic_Depth, Prior_Prediction_Net
...
def main():
    ...
    net = Prior_Prediction_Net().cuda().train()
...
def train(net, optimizer,tb_writer):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            predict_prior= net(inputs)

            loss_net = criterion(predict_prior, mask_dps)
...
def validate(net, curr_iter, optimizer,tb_writer):
    ...
    with torch.no_grad():
        for i, data in enumerate(test1_loader):
            ...
            mask_dps = mask_dps.unsqueeze(1)
            predict_prior = net(inputs)
            loss = criterion(predict_prior, mask_dps)
