...
from nets import basic, depth_predciton, basic_NL, DGNLNet, DensityNet, MyNet, MyNet_DGNL, MyNet_Basic, MyNet_CFT, \
    DDGN_Depth, DDGN_Basic, DDGN_Depth_CFT, DDGN_Density, DDGN_Depth_CFT_Pred, DDGN_Depth_Pred
from config import dataset_dir
from misc import check_mkdir
torch.manual_seed(2019)
# torch.cuda.set_device(0)
ckpt_path = 'ckpt'
exp_name = 'city_depth_pred'
args = {
    'snapshot': 'iter_40000_loss1_0.03264_loss2_0.00000_lr_0.000000',
    'depth_snapshot': ''
}
...
# root = './images'
de_rain_save_root = "results/temp3"
...
list_ssim = []
with torch.no_grad():
    input = open(os.path.join(dataset_dir, 'ImageSets', 'Main', 'val_huxiaowei.txt'))
    gt_input = open(os.path.join(dataset_dir, 'ImageSets', 'Main', 'val_huxiaowei.txt'))
    depth_input = open(os.path.join(dataset_dir, 'ImageSets', 'Main', 'val_huxiaowei.txt'))
    image = [(os.path.join(dataset_dir, 'JPEGImages_DDN', img_name.strip('\n')) + ".png") for img_name in
             input]
    gt = [(os.path.join(dataset_dir, 'Huxiaowei_gt', img_name.strip('\n').split('_leftImg8bit')[0]) + "_leftImg8bit.png") for img_name in
          gt_input]
    depth = [(os.path.join(dataset_dir, 'depth', img_name.strip('\n')) + ".png") for img_name in
             depth_input]
    for idx, data in enumerate(image):
        img = Image.open(image[idx]).convert('RGB')
        gt_img = Image.open(gt[idx]).convert('RGB')
        # depth_img = Image.open(depth[idx].split('_rain')[0].replace('leftImg8bit','depth_rain')+".png")
        w, h = img.size
        ...
        # mask = ((img_var- gt_var)[:, 0, :, :]).type(torch.cuda.FloatTensor)
        # mask = mask.unsqueeze(1)
        # toPIL = transforms.ToPILImage()  # 这个函数可以将张量转为PIL图片，由小数转为0-255之间的像素值
        # pic = toPIL(mask_dps)
        # pic.save('mask.jpg')
        # if "alpha_0.01" in image[idx]:
        #     label = 0
        # elif "alpha_0.02" in image[idx]:
        #     label = 1
        # elif "alpha_0.03" in image[idx]:
        #     label = 2

        # label = torch.Tensor([label]).cuda()
        start_time = time.time()
        # res,depth_pred = net(img_var)
        # print(dps.max(1)[1] + 1)
        ...
        # img_a = cv2.imread(os.path.join(de_rain_save_root,"temp.png"))
