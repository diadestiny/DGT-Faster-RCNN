...
from derain.nets import MyNet_CFT
...
def main(args):
    ...
    de_rain_model = MyNet_CFT()
    de_rain_model.load_state_dict(torch.load("./derain/ckpt/My_Trans_easy/iter_40000_loss1_0.01064_loss2_0.00000_lr_0.000000.pth"))
    # print(models)
    ...
    if args.distributed:
        myModel = torch.nn.parallel.DistributedDataParallel(myModel, device_ids=[args.gpu])
    ...
    if args.test_only:
        utils.evaluate(myModel, data_loader_test, device=device)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mean_loss, lr = utils.train_one_epoch(myModel, optimizer, data_loader, device,
        ...
        coco_info = utils.evaluate(myModel, data_loader_test, device=device)
        ...
        if args.output_dir:
            ...
            save_on_master({
                ...
                'args': args,
                'epoch': epoch},
                os.path.join(args.output_dir, 'new_model_{}.pth'.format(epoch)))

            # save_on_master(de_rain_model.state_dict(), "./multi_train/de_rain-{}.pth".format(epoch))
            # save_on_master(skip_net.state_dict(), "./multi_train/skip_net-{}.pth".format(epoch))
...
if __name__ == "__main__":
    ...
    # 训练文件的根目录(VOCdevkit)
    # parser.add_argument('--gpu', default=[1,4], help='gpu')
    ...
    # 基于上次的训练结果接着训练
    parser.add_argument('--resume', default='./old_models/20211222_39_new_train.pth', help='resume from checkpoint')
