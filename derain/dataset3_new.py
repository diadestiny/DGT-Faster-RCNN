...
def make_dataset(root, is_train,train_str):
    ...
    if is_train:
        input = open(os.path.join(root, 'ImageSets','Main','train_rain.txt'))
        gt_input = open(os.path.join(root, 'ImageSets', 'Main', 'train_rain.txt'))
        depth_input = open(os.path.join(root, 'ImageSets', 'Main', 'train_rain.txt'))
        image = [(os.path.join(root, 'JPEGImages_rain_all', img_name.strip('\n'))) for img_name in
                 input]
        gt = [(os.path.join(root, 'Huxiaowei_gt', img_name.strip('\n').split('_rain')[0])) for img_name in
                 gt_input]
        if train_str == "mask":
            ...
            depth = [(os.path.join(root, 'depth', img_name.strip('\n').split('_rain')[0].replace('leftImg8bit','depth_rain'))) for img_name in
                     depth_input]
            print("depth")
        input.close()
        ...
        input = open(os.path.join(root, 'ImageSets','Main','val_huxiaowei.txt'))
        gt_input = open(os.path.join(root, 'ImageSets', 'Main', 'val_huxiaowei.txt'))
        depth_input = open(os.path.join(root, 'ImageSets', 'Main', 'val_huxiaowei.txt'))
        image = [(os.path.join(root,'JPEGImages_rain_all', img_name.strip('\n'))) for img_name in
                 input]
        gt = [(os.path.join(root, 'Huxiaowei_gt', img_name.strip('\n').split('_rain')[0])) for img_name in
              gt_input]
