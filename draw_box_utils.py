...
def filter_low_thresh(boxes, scores, classes, category_index, thresh, box_to_display_str_map, box_to_color_map):
    ...
    for i in range(boxes.shape[0]):
        if scores[i] > thresh:
            ...
            if class_name not in res.keys():
                res[class_name]=[]
            res[class_name].append(box)
        else:
...
def draw_box(image, boxes, classes, scores, category_index, thresh=0.5, line_thickness=8):
    ...
    jsonData = filter_low_thresh(boxes, scores, classes, category_index, thresh, box_to_display_str_map, box_to_color_map)
