...
from nets import depth_predciton, basic, basic_NL, DGNLNet, MyNet_Basic, DDGN_Basic, Basic
...
train_set = ImageFolder(dataset_dir, transform=transform, target_transform=transform, triple_transform=triple_transform, is_train=True)
train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=8, shuffle=True)
test1_set = ImageFolder(dataset_dir, transform=transform, target_transform=transform, is_train=False)
...
def train(net, optimizer):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            log = '%d epoch:[iter %d], [train loss %.5f], [lr %.13f], [loss_net %.5f]' % \
